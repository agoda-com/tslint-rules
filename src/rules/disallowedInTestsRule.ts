...
export class Rule extends Lint.Rules.AbstractRule {
  public static DEFAULT_FAILURE_STRING =
    'This method invocation is not allowed to be used on test files';

  public apply(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
    return this.applyWithWalker(
      new DisallowedInTestsWalker(sourceFile, this.getOptions()),
    );
  }
}
...
class DisallowedInTestsWalker extends Lint.RuleWalker {
  public visitCallExpression(node: ts.CallExpression) {
    ...
    const verifiedCustomOptions: ICustomOption[] = this.verifyAndGetCustomOptions(
      functionName,
    );
  ...
  private verifyAndGetCustomOptions(functionName: string): ICustomOption[] {
    ...
    const verifiedCustomOptions: ICustomOption[] = customOptionSetInTslintJson.filter(
      option => option && option.name === functionName,
    );
