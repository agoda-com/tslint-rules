...
export class Rule extends Lint.Rules.AbstractRule {
  ...
  public static metadata: Lint.IRuleMetadata = {
    ruleName: 'do-not-use',
    description:
      'Prints out a warning that this function / method should not be used',
    optionsDescription:
      'Please check https://palantir.github.io/tslint/rules/ban/',
    options: {
      type: 'list',
      listType: {
        anyOf: [
  ...
  public static createFailureText(
    expression: string,
    messageAddition?: string,
  ) {
    return `Usage of '${expression}' is strongly discouraged.${
...
function parseOptions(
  args: Array<string | string[] | IOptionsInput>,
): IOptions {
...
class DoNotUseFunctionWalker extends Lint.AbstractWalker<IOptions> {
  ...
  private checkForObjectMethodDoNotUse(
    ...
    for (const ban of this.options.methods) {
      ...
      if (
        ...
        this.addFailureAtNode(
          expression,
          Rule.createFailureText(
            `${ban.object.join('.')}.${ban.name}`,
            ban.message,
          ),
        );
        break;
  ...
  private checkFunctionDoNotUse(name: ts.Identifier) {
    ...
    for (const ban of this.options.functions) {
      if (ban.name === text) {
        this.addFailureAtNode(
          name,
          Rule.createFailureText(text, ban.message),
        );
        break;
      }
    }
